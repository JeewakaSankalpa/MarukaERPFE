...
const getProductsBulk = async (ids) => (await api.post(`/products/ids`, ids)).data;
...
export default function PendingToPOPage() {
    ...
    const [productDetails, setProductDetails] = useState({}); // productId -> ProductDetailsDTO
    ...
    useEffect(() => {
        (async () => {
            try {
                ...
                // Fetch fresh product details to get latest suppliers
                const pids = (p?.lines || []).map(l => l.productId);
                if (pids.length > 0) {
                    const details = await getProductsBulk(pids); // [{id, suppliers:[...]}, ...]
                    const map = {};
                ...
                <Table hover responsive>
                    ...
                    <tbody>
                        {(plan.lines || []).map(l => {
                            ...
                            // Using fresh suppliers from productDetails if available, else fallback to plan snapshot
                            const suppliers = productDetails[l.productId]?.suppliers || l.suppliers || [];
                                        ...
                                        <Form.Select value={c.supplierId || ""} onChange={e => setChoices(s => ({ ...s, [l.productId]: { ...c, supplierId: e.target.value } }))}>
                                            ...
                                            {suppliers.filter(s => s.active !== false).map(s =>
                                                <option key={s.supplierId} value={s.supplierId}>
                                                    {s.supplierName || s.supplierNameSnapshot}{s.lastPurchasePrice ? ` — ${s.lastPurchasePrice}` : ""}
                                                </option>
                                            )}
                                        </Form.Select>
                                    </td>
