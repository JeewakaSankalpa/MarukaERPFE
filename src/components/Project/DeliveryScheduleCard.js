...
export default function DeliveryScheduleCard({ projectId, reloadKey }) {
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            toast.success("Delivery schedule saved. Gate pass and delivery note are available.");
    ...
    const downloadGatePass = async (passNumber) => {
        try {
            const res = await api.get(`/projects/${projectId}/delivery/gatepass/history/${encodeURIComponent(passNumber)}`, { responseType: 'blob' });
            const url = window.URL.createObjectURL(new Blob([res.data]));
            const link = document.createElement('a');
            ...
            toast.error('Failed to download gate pass');
        }
    };
    ...
    return (
        <Card className="shadow-sm mb-4">
            ...
            <Card.Body>
                ...
                {history.length > 0 && <div className="mt-4"><h6>Gate Pass Generation History</h6><Table size="sm" responsive><thead><tr><th>Pass</th><th>Panels / Components</th><th>Generated by</th><th>Generated at</th><th>Documents</th></tr></thead><tbody>{history.map((item, index) => <tr key={`${item.passNumber || 'pass'}-${index}`}><td>{item.passNumber || '-'}</td><td>{(item.componentNames || []).join(', ')}</td><td>{item.generatedBy || '-'}</td><td>{item.generatedAt ? new Date(item.generatedAt).toLocaleString() : '-'}</td><td className="d-flex gap-1"><Button size="sm" variant="outline-secondary" onClick={() => viewGatePass(item.passNumber)}>View gate pass</Button><Button size="sm" variant="outline-danger" onClick={() => downloadGatePass(item.passNumber)}>Download gate pass</Button><Button size="sm" variant="outline-success" onClick={() => downloadPdf('confirmation')}>Download delivery note</Button></td></tr>)}</tbody></Table></div>}
            </Card.Body>
