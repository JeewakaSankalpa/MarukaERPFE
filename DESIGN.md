...
2026-09-05 by Codex: clarified Fulfil Requests item filtering; use a full-width text search below project/requester filters, search request item lines by item name/SKU/barcode/product ID, and avoid dropdowns that depend on preloaded product pages.
2026-09-05 by Codex: expanded Fulfil Requests item-search results to show component allocations beside matched products; component names should be visible wherever request lines are reviewed or fulfilled.
2026-09-07 by Codex: added compact, range-responsive sales pipeline and revenue charts; analytical report visuals use restrained blue, green, amber, and red status colors with print-safe chart framing.
2026-09-07 by Codex: added the Cargills inquiry method as a compact operational split between Materials and Panels estimation/quotation tracks; always show each track's status and total, plus the combined invoice total rule, without blending the two approval histories.
2026-09-07 by Codex: added Cargills-only custom invoice print lines as a compact reconciliation editor; descriptions, quantities, units, and rates are editable, while the protected supply value, VAT, document total, payments, and total due remain system-controlled and must reconcile before save or print.
2026-09-24 by Codex: GRN rate corrections use a two-column operational workspace: receipt batches on the left, a focused rate and VAT editor with a live total preview on the right. Show an exact blocking reason beside the selected batch and keep the Maruka blue palette, compact labels, and full-width mobile stacking.
